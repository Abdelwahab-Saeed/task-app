...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-white">Edit Note</h1>
        </div>
        <div class="mt-4 sm:mt-0 flex gap-2">
            <a href="{{ route('admin.notes.show', $note) }}" class="inline-flex items-center rounded-md bg-[#1A1D24] px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-[#2A2D36] hover:bg-[#2A2D36] transition-colors">
                View
            </a>
            ...
            <div class="px-4 py-5 sm:p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" required
                              class="mt-2 block w-full rounded-md border-0 py-1.5 px-3 text-white bg-[#1A1D24] shadow-sm ring-1 ring-inset ring-[#2A2D36] focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm @error('content') ring-red-500 @enderror">{{ old('content', $note->content) }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="reference_link" id="reference_link" value="{{ old('reference_link', $note->reference_link) }}"
