...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-white">Create Meeting</h1>
        </div>
            ...
            <div class="px-4 py-5 sm:p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="agenda" id="agenda" rows="4"
                              class="mt-2 block w-full rounded-md border-0 py-1.5 px-3 text-white bg-[#1A1D24] shadow-sm ring-1 ring-inset ring-[#2A2D36] focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm @error('agenda') ring-red-500 @enderror">{{ old('agenda') }}</textarea>
                    @error('agenda')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
